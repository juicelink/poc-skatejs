...
export default function register(Component: any, tagName: any) {
	...
	return (<any>document).registerElement(
...
function unRenderElement() {
	render((<any>h)(Empty), this.shadowRoot, this._root);
}
...
function toVdom(element: any, nodeName?: any): any {
	...
	for (i=a.length; i--; ) (<any>props)[a[i].name] = a[i].value;
